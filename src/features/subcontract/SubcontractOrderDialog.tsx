...
interface SubcontractOrderDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function SubcontractOrderDialog({
  open,
  onOpenChange,
}: SubcontractOrderDialogProps) {
  ...
  const groups = groupSubcontractBySupplier(rows);
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      const parsed = parseSubcontractWorkbook(await readExcelBuffer(file));
      setRows(parsed);
      setSelectedIds(new Set());
    } catch (error) {
      toast({
  ...
  const toggleGroup = (groupRows: SubcontractRow[], checked: boolean) => {
    setSelectedIds((current) => {
      const next = new Set(current);
  ...
  const handleOrder = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await placeSubcontractOrders(selectedRows, orgId);
      const created = flattenCreatedBills(result.succeeded);
      if (created.length > 0) {
        setCreatedBills((current) => [...current, ...created]);
        toast({
          title: `已生成 ${created.length} 张委外订单`,
          description: created
  ...
  const handleBillAction = async (action: 'submit' | 'audit' | 'push') => {
    ...
    setOperating(action);
    try {
      const outcome = await operateSubcontractBills(action, selectedBills);
      if (outcome.success) {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-[22px]">委外下单</DialogTitle>
          <DialogDescription>在系统中新增委外订单</DialogDescription>
        </DialogHeader>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-zinc-500">
                        ...
                        <th className="px-3 py-2 font-medium">计划完工时间</th>
                    ...
                    <tbody>
                      {group.rows.map((row) => (
                        <tr key={row.id} className="border-t">
                          ...
                          <td className="px-3 py-2">{row.planFinishDate}</td>
